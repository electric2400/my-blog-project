    ...
    return (
        <main className="min-h-dvh md:w-4xl mx-auto p-3 md:p-6 shadow-sm md:space-y-5">
            ...
            <div className="flex items-center gap-3">
                <img src={blog.image} alt={blog.author.slice(0, 1).toUpperCase()} className="w-8 h-8 rounded-full" />
                <h1>{blog.author}</h1>
            </div>

            <div>
                <h1 className="my-5 font-semibold text-2xl text-gray-800 uppercase">{blog.title}</h1>
                <div className="leading-8"><div dangerouslySetInnerHTML={{ __html: blog.content }}/></div>
            </div>
